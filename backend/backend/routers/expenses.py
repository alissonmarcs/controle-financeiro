...
async def list_expenses(
    ...
    query_result = await db_session.scalars(select(models.Expense).offset(paginator.offset).limit(paginator.limit))
...
async def update_expense(
    ...
    expense = await db_session.scalar(select(models.Expense).where(models.Expense.id == expense_id))
    if not expense:
        raise HTTPException(
